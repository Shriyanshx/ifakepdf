...
export default function PDFViewer({
  ...
  const widthProbeRef = useRef<HTMLDivElement>(null); // invisible div – measures available width
  ...
  const getFitScale = useCallback(
    (naturalPageWidth: number): number => {
      const containerWidth = widthProbeRef.current?.clientWidth ?? 0;
  ...
  const renderPage = useCallback(async (doc: pdfjs.PDFDocumentProxy, idx: number) => {
    ...
    // CSS-space viewport — used for coordinate transforms (bbox ↔ canvas px)
    ...
    // Physical-pixel viewport — what PDF.js actually draws into
    const renderViewport = page.getViewport({ scale: s * dpr });
    ...
    try {
      ...
      // Ignore cancellation errors — a new render is already queued
  ...
  useEffect(() => {
    ...
    if (!probe) return;
    let lastWidth = probe.clientWidth;
    const ro = new ResizeObserver(() => {
      const newWidth = probe.clientWidth;
      // Only re-render when width meaningfully changes (ignore height-only changes)
  ...
  return (
    <div className="flex flex-col items-center gap-4 w-full">
      {/* Invisible width probe — sits at top, zero height, always full width */}
      <div ref={widthProbeRef} className="w-full" style={{ height: 0, overflow: 'hidden' }} />
      {/* Hint text */}
      {previewOverlay ? (
